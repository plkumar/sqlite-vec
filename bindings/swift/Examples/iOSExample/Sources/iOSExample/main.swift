...
class iOSVectorSearchExample {
    ...
    init() throws {
        ...
        // Note: Auto-loading is deprecated on Apple platforms, so we load manually for each connection
    ...
    private func setupDatabase() throws {
        ...
        try db.execute("""
            CREATE VIRTUAL TABLE IF NOT EXISTS app_content USING vec0(
                embedding float[384],
                id TEXT,
                title TEXT,
                content TEXT,
    ...
    func addContent(_ items: [(id: String, title: String, content: String, category: String, embedding: [Float32])]) throws {
        ...
        for item in items {
            try db.run("""
                INSERT INTO app_content (id, title, content, category, created_at, embedding)
                VALUES (?, ?, ?, ?, ?, ?)
            """, item.id, item.title, item.content, item.category, currentTimestamp, Blob(bytes: [UInt8](SQLiteVec.serialize(item.embedding))))
        }
    ...
    func searchSimilar(to query: [Float32], category: String? = nil, limit: Int = 10) throws -> [(id: String, title: String, content: String, similarity: Double)] {
        ...
        var parameters: [Binding?] = [Blob(bytes: [UInt8](SQLiteVec.serialize(query)))]
    ...
    func getStats() throws -> (totalItems: Int, categories: [String]) {
        ...
        let categories = categoryResults.map { row in
            row[0] as! String
        }
